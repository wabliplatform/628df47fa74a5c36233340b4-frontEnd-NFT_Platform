...
if(insideSubDataElement !== null && data[data.length -i -1].NFTimage !== undefined){
  ...
  subDataElements[i-(chunk-1) *subDataElements.length].name = data[data.length -i -1].NFTimage.name;
      ...
      if(data[data.length-i-1].NFTCategory._id){
        ...
        const insideSubdocument = subDataElements[i-(chunk-1) * subDataElements.length].querySelector("[annotationname = 'UserNFTs']");
        if (insideSubdocument !==null) {
          const tableData = data[data.length - i - 1].UserNFTs;
    ...
    tableData.forEach((data,index) => {
      ...
      const attributeSubdocumentElement = tableDataElement[
        index
      ].querySelector("[annotationname = 'NFTdescription']");
      if (attributeSubdocumentElement !== null) {
        attributeSubdocumentElement.textContent = tableData[tableData.length - index -1].NFTdescription;
      }
    }
    catch(e) {console.log(e);}; try {
      const attributeSubdocumentElement = tableDataElement[
        index
      ].querySelector("[annotationname = 'NFTCategory']");
      if (attributeSubdocumentElement !== null) {
        attributeSubdocumentElement.textContent = tableData[tableData.length - index -1].NFTCategory;
      }
    }
      ...
      if(data[data.length-i-1].UserNFTs._id){
        map.set(
           subDataElements[i-(chunk-1) * subDataElements.length].querySelector(
            "[annotationname = 'UserNFTs']"
          ).getAttribute("id"),
          data[data.length-i-1].UserNFTs
        );
      }
     } catch (e) { console.log(e) };
